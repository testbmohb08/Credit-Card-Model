...
with open('CC.Transactions.csv','w') as f1:
	...
	generate_cc_data(Merchant_Category_Green,Country_Green,4,-2,5,count)
	...
	#generate amount for current transaction with 50%-50% distribution on credits and debits
	generate_cc_data(Merchant_Category_Yellow,Country_Yellow,3,-1,25,count)
	...
	#generate amount for current transaction with 50%-50% distribution on credits and debits
	generate_cc_data(Merchant_Category_Green,Country_Green,3,-1,400,count)
	...
	#generate amount for current transaction with 50%-50% distribution on credits and debits
	generate_cc_data(Merchant_Category_Red,Country_Red,3,-1,5,count)
	
	generate_cc_credits(Merchant_Category_Green,5,count)
	
	#Red Transactions with payments from non BMO entities 
	gen_cc_external(Merchant_Category_Red,5,count)
